...
function plugin(opts){
  ...
  return function(files, metalsmith, done) {
    ...
    function check(file, frontmatter) {
      ...
      // Amp Files
      if (file.includes('/amp/index.html')) {
        return false;
      }
    ...
    Object.keys(files).forEach(function(file) {
      ...
      // entry.lastmod を url から解析
      // https://anz-note.firebaseapp.com/articles/20190101/misc-generate-by-metalsmith/
      const arr = entry.url.split('/')
      if (arr.length == 4) {
        const publishDate = arr[1];
        if (publishDate.length == 8 && !isNaN(parseInt(publishDate, 10))) {
          const yyyy = publishDate.substr(0, 4)
          const mm = publishDate.substr(4, 2)
          const dd = publishDate.substr(6, 2)
